...
public:
    ...
    void push_cand(Candidate cand) {
        this->players.back().push_back(cand);
    }
};
...
std::deque<std::deque<unsigned int>> compute_rk(unsigned int r0){
    ...
    while (r_new != 1){
        r_last = r[r.size() - 1];
        k_last =
            2 * (r_last - (int) pow(2, (int)(floor(log2(r_last - 1)))));
...
void compete(std::list<Candidate> node1,
             std::list<Candidate> node2, 
             Round& round){
    ...
    for (auto cand1: node1){
        ...
        round.push_cand(
            Candidate(cand1_wgt,
            posterior_prob * cand1.prob * cand1_wgt));
        return;
    }
}
