...
void make_arr(std::string big_str, std::size_t big_str_size, StringArray* str_arr_object);
...
int main()
{
	...
	else { fill_arr(arr, MAX_SIZE, &used_size, &std::cin); }
...
void fill_arr(int arr[], int size, int* used_size, std::istream* input_place)
{
	...
	for (int i = 0; i < (*used_size); i++)
		arr[i] = get_number(str_arr_obj.str_arr[i],
				            str_arr_obj.sizes_arr[i]);
	return;
}
...
void print_results(const int arr[], const int count_arr[], int size)
{
	...
	std::cout << std::setw(5) << "N" << "    " << "Count\n";
	std::cout << std::setw(5) << arr[0] << "    "
		<< count_arr[0] << '\n';
	for (int i = 0; i < size; i++)
	{
