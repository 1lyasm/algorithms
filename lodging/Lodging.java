...
public class Lodging {
        ...
        for (Map.Entry<Personnel, Apartment> entry: hm.entrySet()) {
            ...
            if (p != null && a != null) {
                System.out.println("P_Id:" + p.getId() +
                                " personel lives in -> Apartment " + a.getId());
            }
        }
    }
    ...
    public void returnApartment(Personnel personnel) {
        ...
        if (!this.hm.containsKey(personnel)) {
            System.out.println(personnel.getId() + "," + personnel.getName() +
                    " has already returned the daire.");
        }
        boolean found = false;
        for (Map.Entry<Personnel, Apartment> entry: hm.entrySet()) {
            if (!found) {
                ...
                if (personnel.getId() == p.getId()) {
                    if (a == null) {
                        ...
                        if (waiting_p != null) {
                            ...
                            System.out.println("P_Id:" + p.getId() +
                                    " personel returned the Aprtmnt_Id: " + a.getId() +
                                    " and new personnel was placed in it.");
                            found = true;
                        }
                        else {
                            existing_personnel = p;
    ...
    public void newApartment() throws ApartmentException {
        ...
        if (waiting_p == null) {
            System.out.println("There is no one waiting for apartment, apartment id: " + id + ", type: " + type);
        } else {
    ...
    private void placement(Personnel waiting_p, Apartment a) {
        ...
        System.out.println("P_Id:" + waiting_p.getId() +
                " was placed in Aprtmnt_Id: " +  a.getId());
        System.out.println("P_Id:" + waiting_p.getId() +
                " personel lives in -> Apartment " +  a.getId());
    }
}
