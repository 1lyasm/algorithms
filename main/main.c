...
int numIslands(char **grid, int Size, int *ColSize) {
	int m = (*ColSize); int *idx_stack = malloc(2 * sizeof(int) * Size * m);
	int sp; int Cnt = 0;
	for (int i = 0; i < Size; i++) {
		for (int j = 0; j < m; j++) {
			int tmp = grid[i][j] - '0'; Cnt += tmp;
			idx_stack[0] = i; idx_stack[1] = j; sp = 2 * tmp;
			for (; sp > 0;) {
				int x = idx_stack[sp - 2], y = idx_stack[sp - 1];
				grid[x][y] = '0'; sp -= 2; idx_stack[sp] = x;
				idx_stack[sp + 1] = y + 1;
				sp += (2 * (((y + 1) < m) && (grid[x][y + 1] - '0')));
				idx_stack[sp] = x; idx_stack[sp + 1] = y - 1;
				sp += (2 * (((y - 1) >= 0) && (grid[x][y - 1] - '0')));
				idx_stack[sp] = x + 1; idx_stack[sp + 1] = y;
...
void partitionLabelsTest() {
	...
	int returnSize; int* Res = partitionLabels(s, &returnSize);
	free(Res);
}
...
int* twoSum(int* nums, int Size, int target, int* returnSize) {
	...
	for (int i = 0; i < Size; i++) {
		...
		if (elem != reduced_arr[reduced_size - 1]) {
			reduced_arr[reduced_size] = elem; reduced_size++;
		}
	}
	free(arr); arr = 0; int* indices = 0;
	for (int i = 0; i < reduced_size; i++) {
		int key = target - reduced_arr[i];
		void* search_Res = bsearch(&key, reduced_arr,
			reduced_size, sizeof(int), twoSumSearchCmp);
	...
	free(reduced_arr); reduced_arr = 0;
	return indices;
}
...
struct llist_node* addTwoNums(struct llist_node* l1, struct llist_node* l2) {
	...
	cur->val = sum % 10; l1 = l1->next; l2 = l2->next;
	while (l1 || l2 || carry) {
		sum = carry; if (l1) { sum += l1->val; l1 = l1->next; }
...
void addTwoNumsTest() {	
	...
	for (int i = 0; i < len1 - 1; i++) {
		a->val = arr1[i]; a->next = malloc(sizeof(struct llist_node));
		a = a->next;		
	}
	a->val = arr1[len1 - 1]; a->next = 0;
	for (int i = 0; i < len2 - 1; i++) {
		b->val = arr2[i]; b->next = malloc(sizeof(struct llist_node));
		b = b->next;		
	}
	b->val = arr2[len2 - 1]; b->next = 0;
	struct llist_node *res = addTwoNums(l1, l2);
	while (res) { struct llist_node *next = res->next; free(res); res = next; }
	while (l1) { struct llist_node *next = l1->next; free(l1); l1 = next; }
	while (l2) { struct llist_node *next = l2->next; free(l2); l2 = next; }
}

void mergeSort(int *Arr, int S, int E) {
	if (E - S <= 1) { merge(Arr, S, E); return; }
	int RS = (S + E + 1) / 2;
	mergeSort(Arr, S, RS - 1); mergeSort(Arr, RS, E); merge(Arr, S, E);
}
...
void merge(int *Arr, int S, int E) {
	...
	int I = E - S, L = M, R = E;
	for (; I >= 0 && L >= S && R >= M + 1; --I) {
		if (Arr[R] > Arr[L]) ExtraArr[I] = Arr[R--];
		else ExtraArr[I] = Arr[L--];
	}
	int Longer, Stop;
	if (L >= S) { Longer = L; Stop = S; }
	else { Longer = R; Stop = M + 1; }
...
void mergeSortTest() {
	printf("\nmergeSortTest: ");
	{
		...
		mergeSort(Inp, 0, Len - 1); assertArrEq(Inp, Correct, Len);
	} {
...
void quickSort(int *Arr, int S, int E) {
	...
	swap(Arr, Pivot, E); int NewPivot = -1;
	for (int L = S, R = E - 1;;) {
		for (; Arr[L] <= PivotVal && L <= E - 1; ++L);
		for (; Arr[R] >= PivotVal && R >= L + 1; --R);
		if (L >= E) { NewPivot = E; break; }
		else if (R <= L) { swap(Arr, L, E); NewPivot = L; break; }
		swap(Arr, L, R);
	}
	quickSort(Arr, S, NewPivot - 1); quickSort(Arr, NewPivot + 1, E);
}
...
void quickSortTest() {
	printf("\nquickSortTest: ");
	{
		int Inp[6] = {1, 4, 9, 2, 3, 8}, Correct[6] = {1, 2, 3, 4, 8, 9};
		int Len = sizeof(Inp) / sizeof(Inp[0]);
		quickSort(Inp, 0, Len - 1); assertArrEq(Inp, Correct, Len);
	} {
		int Inp[6] = {1, 2, 3, 4, 5, 6}, Correct[6] = {1, 2, 3, 4, 5, 6};
		int Len = sizeof(Inp) / sizeof(Inp[0]);
		quickSort(Inp, 0, Len - 1); assertArrEq(Inp, Correct, Len);
...
struct Arr { int Len; int64_t *ArrPtr; };

struct Arr* genArmstrong(int N) {
	int ResLen = 0; struct Arr *Res = malloc(sizeof(struct Arr*));
	Res->ArrPtr = malloc(N * sizeof(int));
	for (int Num = 0; Num < N; ++Num) {
		int Sum = 0, MutatedNum = Num, Last = Num % 10;
		Sum += Last * Last * Last; MutatedNum /= 10; Last = MutatedNum % 10;
		Sum += Last * Last * Last; MutatedNum /= 10; Last = MutatedNum % 10;
		Sum += Last * Last * Last; if (Sum == Num) Res->ArrPtr[ResLen++] = Num;
	}
	Res->Len = ResLen;
	return Res;
}
...
void genArmstrongTest() {
	...
	free(Res->ArrPtr); Res->ArrPtr = 0; free(Res); Res = 0;
	printf("PASSED\n");
}
...
int64_t* genNarc(int64_t N) {
	...
	for (int NumLen = 2; NumLen < DigitCnt; ++NumLen) {	
		...
		for (int64_t Sum = 0;;) {
			...
			for (; Sum > 0 && SumLen < NumLen;
				SumArr[SumLen++] = Sum % 10, Sum /= 10);
			if (Sum == 0 && SumLen == NumLen) {
				qsort(SumArr, NumLen, sizeof(int), genNarcCmp);
				if (memcmp((const void*) SumArr, (const void*) Num, 
					NumLen * sizeof(int)) == 0) Res[ResLen++] = SumCopy;
 			}
			free(SumArr); SumArr = 0;
			if (++Num[0] == 10) {
				int J = 1; for (; J < NumLen && Num[J] == 9; ++J);
				if (J == NumLen) { break; }
...
int genNarcCmp(const void *a, const void *b) {
	int arg1 = *(const int*)a; int arg2 = *(const int*)b;
    if (arg1 < arg2) return 1; if (arg1 > arg2) return -1;
    return 0;
}

int genNarcCmpRev(const void *a, const void *b) {
	int64_t arg1 = *(const int64_t*)a; int64_t arg2 = *(const int64_t*)b;
    if (arg1 < arg2) return -1; if (arg1 > arg2) return 1;
    return 0;
}
...
void genNarcTest() {
	...
	int64_t Correct[] = {1, 2, 3, 4, 5, 6, 7, 8, 9, 153, 370, 371,
		407, 1634, 8208, 9474, 54748, 92727,
		93084, 548834, 1741725, 4210818, 9800817,
...
void ToH(struct Sticks *Towers, int Source,
	int Destination, int Count, int N) {
	assert(Source <= 2); 
	assert(Source >= 0);
