...
public:
    ...
    bool continues(void);
    int search_str_arr(std::string target,
...
{
    for (int i = 0,
        month_counter = model.month_index;
...
void Controller::file_inp_month_index(int& month_index)
{
    ...
    month_index = search_str_arr(month,
        Model::MONTHS,
        Model::N_MONTH);
}
...
void Controller::inp_monthly(Model& model)
{
        ...
        i < Model::N_MONTH; i++, month_counter++)
    {
        if (month_counter >= Model::N_MONTH)
            month_counter = 0;
        std::cout << "Rainfall for "
            << Model::MONTHS[month_counter]
            << "(ex: 300): ";
...
int Controller::search_str_arr(std::string target,
    const std::string arr[],
    int size)
{
...
public:
    ...
    View(Model* model_ptr_arg)
    {
        View::model_ptr = model_ptr_arg;
    };
    ...
    void copy_arr(const double source_arr[],
        double destination_arr[],
        int size);
};
...
void View::print_table(void)
{
        ...
        i++, month_counter++)
    {
        ...
        View::multiple_print(' ',
            static_cast<int>(View::TABLE_WIDTH * 0.4));
        (*View::ostream_ptr) << std::setw(10) << (*View::model_ptr).monthly[i];
        View::multiple_print(' ',
            static_cast<int>(View::TABLE_WIDTH * 1.4));
...
{
    (*View::ostream_ptr) << std::setw(field_width)
        << Model::MONTHS[month_index] << " | ";
    return;
}
...
int View::scale(double num)
{
    ...
    if (int_num % View::SCALE_FACTOR >=
        View::SCALE_FACTOR / 2)
        return (int_num / View::SCALE_FACTOR + 1);
    else { return (int_num / View::SCALE_FACTOR); }
}

void View::copy_arr(const double source_arr[],
    double destination_arr[],
    int size)
{
