...
void fillInsertPos(std::vector<int>& insertPos,
	std::vector<int> blankPos, std::vector<int> gapLengthArr) {
...
void fillGapLengthArr(std::vector<int>& gapLengthArr,
	std::vector<int> blankPos, std::string line) {
...
void fillMultipleLines(std::vector<std::string>& multipleLines,
	...
	for (int i = 0; i < insertPos.size() && insertPos[i] >= 0; i++)
		filledSize++;
	for (; i < filledSize; i++) {
		multipleLines[i] = line.substr(leftSideIndex, insertPos[i] - leftSideIndex + 1);
	...
	for (int i = 0; i < multipleLines.size(); i++) {
		...
		if (deleteAmt > 0) {
			...
			if (multipleLines[i][multipleLines[i].size() - 1] == '\n')
				multipleLines[i].erase(multipleLines[i].begin() + multipleLines[i].size() - 1);
		}
	}
}

void copyWithLineNumber(std::ifstream* iFile,
	std::ostream* oStream)
{
	char c{};
	...
	while (1) {
		...
		if (c == '\n') {
			if (line.size() >= 72) {
				...
				for (int i = 0; i < multipleLines.size() &&
					 multipleLines[i] != "null"; i++)
